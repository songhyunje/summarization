...
class MultiBertAbs(nn.Module):
    ...
    def forward(self, sources, target):
        ...
        for src in sources:
            ...
            # print(src["input_ids"].size())  # batch x source_seq
            # print(encoder_output[-1].size())  # batch x source_seq x 768

        # encode_input_ids: batch x source_seq
        encoder_input_ids = torch.cat(encoder_input_ids, dim=1)
        # print(encoder_input_ids.size())
        # encoder_hidden_states: batch x source_seq x 768
