...
from src.common.dataset.dataset import MDSDataset
from src.common.tokenization.tokenization_kt import Tokenizer
from src.multi_summary.multibertabs import MultiBertAbs
...
class MDS(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.tokenizer = Tokenizer(os.path.join(self.hparams.model_name_or_path,
                                   ...
                                   max_len=512)
        self.model = MultiBertAbs(self.hparams)
    ...
    def forward(self, batch):
        outputs = self.model(batch['sources'], batch['target'])
    ...
    def load_and_cache_data(self, data_type):
        ...
        cached_file = os.path.join(
            ...
            "cached_{}_{}_{}_{}_{}".format(
                ...
                str(self.hparams.max_src_sent_length),
        ...
        if os.path.exists(cached_file) and not self.hparams.overwrite_cache:
            ...
            # dataset = MDSSSDataset(
            #     self.tokenizer, data_dir=self.hparams.data_dir, type_path=data_type,
            #     max_src_seq_length=self.hparams.max_src_seq_length,
            #     max_src_sent_length=self.hparams.max_src_sent_length,
            #     max_tgt_seq_length=self.hparams.max_tgt_seq_length
            # )
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument(
            ...
            default=256,
            type=int,
            help="The maximum total one input sequence length after tokenization. Sequences longer "
                 "than this will be truncated, sequences shorter will be padded.",
        )
        parser.add_argument(
            "--max_tgt_seq_length",
